...
internal class OutputTemplateRenderer
{
    ...
    public OutputTemplateRenderer(string outputTemplate,
    ...
    public object[] Format(LogEvent logEvent)
    {
        ...
        foreach (var renderer in _renderers) renderer.Render(logEvent, buffer.Add);

        return buffer.Where(item => item != null).Select(item => item!).ToArray();
    }
}
